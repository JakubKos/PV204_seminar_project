...
// MODIFIED VERSION INTENDED FOR TESTING PURPOSES, CAN MAKE WC_RNG LOAD DATA
// FROM /tmp/random IF CUSTOM_RNG ENVIRONMENT VARIABLE IS SET TO 1
...
{
    char* CUSTOM_RNG = getenv("CUSTOM_RNG");
    if(CUSTOM_RNG && CUSTOM_RNG[0] != '0' && CUSTOM_RNG[0] != 'F') {
        FILE* f = fopen("/tmp/random", "rb");
